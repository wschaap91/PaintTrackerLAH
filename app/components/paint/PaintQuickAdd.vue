...
async function lookup(query: { code?: string, barcode?: string }) {
  ...
  try {
    ...
    if (result) {
      ...
      lookupError.value = 'No paint matches that code. Try a different code or use the full Add Paint page.'
    }
  }
  catch {
    lookupError.value = 'Catalog lookup failed. Try a different code or use the full Add Paint page.'
  }
}
  ...
  <div class="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" @click.self="emit('close')">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex border-b border-gray-100">
        <button
          v-for="tab in ['code', 'scan'] as const"
          :key="tab"
