...
Last updated: 2026-06-02 (after PR #135 — v8 Wave 2: PaintCardCompact icons, paints page integration)
...
- `addFromCatalog({ catalogPaintId, status?: 'owned' | 'wishlist' })` → `Id<"paints">` — auth required; creates paint from catalog entry with given status (default `'owned'`); throws if user already owns a paint with same `catalogPaintId`
...
- **Paints index tabbed interface**: `paints/index.vue` is a three-tab UI (All / Owned / Wishlist). All tab: catalog browsing via `useCatalogBrowse()` with IntersectionObserver infinite scroll sentinel. Owned/Wishlist tabs: client-side filtering from `usePaints()` (fetches all, filters by `status`). `PaintSearchBar` is shared across tabs; brand filter uses `searchFilters.value.brand` for all tabs (no per-tab state — brand persists across tab switches). `brandsForCurrentTab` computed returns catalog-only brands on All tab and a deduplicated union of catalog + user brands on Owned/Wishlist tabs. `paintType` is synced to `catalog.filters.colorFamily` on tab switch and search update when on All tab. `PaintTabPills` shows unfiltered counts. `PaintCardCompact` used across all tabs (check icon `w-6 h-6 text-accent-500`; heart icon `w-6 h-6 text-pink-300`). Owned tab cards pass `:is-owned="true"` and `:is-wishlisted="paint.status === 'wishlist'"`. Wishlist tab cards pass `:is-owned="false"` and `:is-wishlisted="true"`. Page title + `ThreeDotMenu` in a sticky auto-hide container driven by `useScrollHeader()` (`transition-transform`, `-translate-y-full` on scroll down). Search bar + `PaintTabPills` in a separate sticky container (`sticky z-30 bg-gray-50`) with dynamic top offset based on `isHeaderVisible`. Toggle handlers cover add, remove, and switch-status flows with mutation errors surfaced via a `mutationError` ref. `catalogPaintStatusMap` computed tracks owned/wishlisted status for each catalog paint displayed in the All tab.
- **Error handling**: try/catch/finally with local `error` ref + `isLoading` ref in page components; mutation errors surfaced via dedicated `mutationError` ref (separate from query-level error)
...
app/
  ...
  composables/    useAuth.ts, useCatalogBrowse.ts, useCatalogSearch.ts, useConvex.ts,
                  ...
                  useScrollHeader.ts, useShoppingList.ts
  layouts/        default.vue
